...
$sql = "SELECT * FROM register WHERE email ='".$_SESSION['sess_email']."'";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
        echo "<table><center><tr><td>User Name :    </td><td>".$row["name"]."</td></tr><tr><td>Email :    </td><td>".$row["email"]."</td></tr><tr><td>Address :    </td><td>".$row["address"]."</td></tr><tr><td>Phone Number :    </td><td> ".$row["number"]."</td></tr></center></table>";
...
$conn->close();
    ...
    ?>
            ...
            <div class="article">
          <h2><span>Succeessfully Purchase Ticket</span></h2>
          <hr class="w1" align="left" width="90%" size="5" noshade="noshade"/>
          <br>
          <?php
...
$sql = "SELECT * FROM passenger WHERE name ='".$_SESSION['sess_name']."'";
$result = $conn->query($sql);
